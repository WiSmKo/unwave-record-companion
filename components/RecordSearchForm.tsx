...
export default function RecordSearchForm({ onRecordSearch} : RecordSearchFormProps){
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) =>{
    ...
    if (!data.artist && !data.title && !data.catno) {
      // Set a custom error message for each field
      if (!data.artist) {
  ...
  return (
        <form onSubmit={handleSubmit(onSubmit)} >
          <div>
            <label>Artist:
                <input type="text" className="form-input block" placeholder="Band or musician name" {...register("artist")}></input>
                {errors.artist && <p className="text-sm text-red-500">{errors.artist.message}</p>}
            </label>
            <label>
                Title:
                <input type="text" className="form-input block" placeholder="Album/single/record name" {...register("title")}></input>
            ...
            <label>
                Record Label:
                <input type="text" className="form-input block" placeholder="Record company name" {...register("label")}></input>
            </label>
            <label>
                Country:
                <input type="text" className="form-input block" placeholder="Country your copy was made" {...register("country")}></input>
            </label>
            <label>
                Catalouge Number:
                <input type="text" className="form-input block" placeholder="Catalouge number" {...register("catno")}></input>
